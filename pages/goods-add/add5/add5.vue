...
<script>
	export default {
		...
		onLoad:function(option){
			...
			if(this.imageList.detailsImage != ''){
				this.imageUrl=this.imageList.detailsImage.split(',');	
				if(this.imageUrl.length==50) this.hasChooseImage=false;
			}
		},
		methods:{
			navigate:function(){
			...
			update:function(){
				...
				uni.request({
					...
					data: {goodsID:this.goodsID, imageUrl:this.imageUrl.join(','),step:'3'},
					success: (res) => {																	
						console.log('发布成功');		
						uni.hideLoading()
			...
			chooseImage: async function () {
				uni.chooseImage({
					...
					sizeType: ['compressed'],
					count: 50-this.imageUrl.length,
					success: (res) => {
						this.imageUrl =this.imageUrl.concat(res.tempFilePaths);
						if(this.imageUrl.length === 50){
